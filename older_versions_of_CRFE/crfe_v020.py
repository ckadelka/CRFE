...
#v020:  to do:
'''
1. get rid of categories of perturbed genes, each is its own (get rid of self.nr_categories) - DONE
2. implement get_score (make new class variables)
3. keep track of proposals
4. edit prpoposal and undo proposal function
'''
...
class CRFE:
    ...
    def propose_MCMC_state(self,neighborhood_size):
        ...
        if r > self.proportion_parameter_change: #add, delete, or switch a term
            ...
            elif proposal - self.n_terms < self.number_of_unselected_terms * self.number_of_selected_terms: #switch two terms
            ...
            else: #switch ranking of two terms in the current set, new in v020
                #the next three lines implement efficiently drawing without replacement, i.e., np.random.choice(number_selected_sets,2,False)
                r = int(random.random()*self.number_of_selected_terms*(self.number_of_selected_terms-1)//2)
                selected_term_pos1 = r//(self.number_of_selected_terms-1)
                selected_term_pos2 = (1+r%(self.number_of_selected_terms-1))%self.number_of_selected_terms
                
                self.term_id_to_switch1 = self.internal_list_of_terms_for_MCMC[selected_term_pos1]
                self.term_id_to_switch2 = self.internal_list_of_terms_for_MCMC[selected_term_pos2]
                
                self.switch_ranking(self.term_id_to_switch1,self.term_id_to_switch2)
            ...
            if r < self.proportion_parameter_change/3 and self.LEARN_BELIEF or r < self.proportion_parameter_change/2 and self.LEARN_BELIEF==False: #change alpha
                ...
                self.NP_log_alpha = 000000#np.sum(self.log_alphas[self.n_explained_perturbed_genes_per_category==0])#np.dot(self.log_alphas,self.n_perturbed_genes_per_category-self.n_explained_perturbed_genes_per_category)                    
                ...
                self.EP_log_1_minus_beta = 0000#np.dot(self.log_1_m_betas,self.n_explained_perturbed_genes_per_category)
                ...
                if s == self.n_perturbed_genes:
                    self.NP_log_alpha = 0000#np.sum(self.log_alphas[self.n_explained_perturbed_genes_per_category==0])#np.dot(self.log_alphas,self.n_perturbed_genes_per_category-self.n_explained_perturbed_genes_per_category)                    
                    self.EP_log_1_minus_beta = 0000#np.dot(self.log_1_m_betas,self.n_explained_perturbed_genes_per_category)
                else:
                    self.NP_log_alpha = 0000#pass#xxx#recompute sum(self.log_alphas)#sum([a*(b-c) for a,b,c in zip(self.log_alphas,self.n_perturbed_genes_per_category,self.n_explained_perturbed_genes_per_category)])#np.dot(self.log_alphas,self.n_perturbed_genes_per_category-self.n_explained_perturbed_genes_per_category)
                    self.EP_log_1_minus_beta = 0000#sum([a*b for a,b in zip(self.log_1_m_betas,self.n_explained_perturbed_genes_per_category)])#np.dot(self.log_1_m_betas,self.n_explained_perturbed_genes_per_category)
                self.parameter_changed = 'belief'


    def new_gene_annotated_to_selected_terms(self,gene):
        if gene < self.n_perturbed_genes: #perturbed gene
            self.list_of_perturbed_explained_genes.append(gene)                   
        else: #unperturbed gene
            self.IU -= 1
            self.list_of_unperturbed_explained_genes.append(gene) 
    ...
    def gene_no_longer_annotated_to_selected_terms(self,gene):           
        ...
        if category_nr==-1:#gene is unperturbed
            ...
            pass#self.n_explained_perturbed_genes_per_category[category_nr]-=1
            self.EP_log_1_minus_beta-=self.log_1_m_betas[category_nr]
            self.NP_log_alpha+=self.log_alphas[category_nr]   

    def add_term(self,term_id_to_add): #adds a term as the lowest activated term
        ...
        for gene in self.T[term_id_to_add]: 
            ...
            if self.positions_of_selected_terms_each_gene_is_annotated_to[gene] == []:
                self.new_gene_annotated_to_selected_terms(gene)

            #add the newly added term at the end of the ordered list of selected terms the gene is annotated to
            self.positions_of_selected_terms_each_gene_is_annotated_to[gene].append(self.number_of_selected_terms)
        ...
        #update log_beta_is, log_1_minus_beta_is, log_p_ijs and recompute self.log_score_APs, self.log_score_AUs
        self.update_all_beta_scores()

    def remove_term(self,term_id_to_remove): #deletes a term
        ...
        if self.number_of_selected_terms > 0: #otherwise, already sorted the way we want it
            ...
            self.internal_list_of_terms_for_MCMC.pop(pos)
            self.internal_list_of_terms_for_MCMC.insert(self.number_of_selected_terms,term_id_to_remove)
            
            self.position_of_term_in_internal_list[term_id_to_remove] = self.number_of_selected_terms
            for i in range(pos,self.number_of_selected_terms): #all selected terms below term_id_to_remove get moved up by one
                self.position_of_term_in_internal_list[self.internal_list_of_terms_for_MCMC[i]] -= 1
        self.number_of_unselected_terms += 1
    ...
    def get_score(self):
        '''maximize [sum_i EP_i * (1-beta_i)] + [sum_i NP_i * alpha_i] + sum_i A_iU * beta_i + NU *(1-alpha)
        with respect to self.selected terms, alpha, beta and prob'''
        
        L = self.log_score_APs + self.log_score_IPs + self.log_score_AUs + math.log(1-self.alpha) * self.IU
    ...
    def update_all_beta_scores(self):
        for gene in range(self.n_perturbed_genes):#go through all perturbed genes
            try:
                pos = self.positions_of_selected_terms_each_gene_is_annotated_to[gene]
                self.log_score_APs += self.log_p_ijs[pos,gene] + self.log_1_m_betas[pos]
            except IndexError: #perturbed gene not annotated
                pass
    ...
    def initialize(self,initial_set_of_terms=[]):
        ...
        self.positions_of_selected_terms_each_gene_is_annotated_to = [[] for i in range(self.n_genes)]
        self.list_of_unperturbed_explained_genes = []
        self.list_of_perturbed_explained_genes = []
        
        if self.proportion_parameter_change>0: #xxxxxxx do this later
            ...
            for i in range(self.B):
                ...
                possible_alphas_and_betas=np.array([self.get_specific_rates(self.possible_values_for_alpha_and_beta[i]) for i in range(self.A)]) #xxxxx
            ...
            alphas=self.get_specific_rates(self.alpha,self.nr_perturbed_genes,self.belief_alpha) #xxxxx, add number of rates
            betas=self.get_specific_rates(self.beta,self.belief_beta) #xxxxx
        ...
        #this list keeps track of which terms are currently selected (the first self.number_of_selected_terms) and of their relative ranking
        ...
        self.log_score_APs = 0 
        self.log_score_IPs = sum(self.log_alphas)
        self.log_score_AUs = 0
        self.IU = self.n_unperturbed_genes
    ...
    def MCMC_alg(self,initial_set_of_terms=[],verbose=0):
        ...
        neighborhood_size = self.n_terms + self.number_of_unselected_terms * self.number_of_selected_terms + self.number_of_selected_terms * (self.number_of_selected_terms-1) // 2
        ...
        while self.step < total_steps:              
            ...
            if self.term_id_to_toggle!=-1:
                new_neighborhood_size = self.n_terms + self.number_of_unselected_terms * self.number_of_selected_terms + self.number_of_selected_terms * (self.number_of_selected_terms-1) // 2
            ...
            if self.score > self.max_score:
                ...
                if self.LEARN_PENALIZATION_PARAMETER:
                    ...
                    self.max_score_prob = 0
                self.best_set = self.internal_list_of_terms_for_MCMC[:self.number_of_selected_terms]
    ...
    def mean_position_perturbed_genes(self,list_of_terms):
        ...
        for term in list_of_terms:
            if self.T[term][0]>=self.n_perturbed_genes: #if no perturbed gene is annotated to the term
                ...
                index = sum(self.T[term] < self.n_perturbed_genes)
    ...
    def get_filename(self,method,LONG=False):
        filename='%s_random%s_param_learning%s_k%s_%sto%sannotations' % (self.out_str,int(1000*method[1]),int(self.proportion_parameter_change*1000),self.lower_cutoff,self.upper_cutoff)
        if LONG:
            filename+='_belief%s_nr_perturbed%s' % (int(self.belief*1000),int(self.n_perturbed_genes))
    ...
    def runMe(self, verbose=False, parameter_initial_MCMC_set=0):        
        ...
        #If a gene is in annotation data but not in expression data, then an error occurs!
        ...
        if self.threshold_type=='proportion':
            self.n_perturbed_genes = int(self.n_genes * self.threshold)
        else:
            self.n_perturbed_genes = sum(self.levels >= self.threshold)
        ...
        for i in range(len(self.terms)):
            if avg_posteriors[i]==0: #stop when the posterior is 0
                break
...
def main():
    ...
    p.add_option('--threshold_type', '-T', default='proportion', help="Whether threshold should be interpreted as 1. a value, or 2. proportion ('proportion') of perturbed genes, (default proportion)")
