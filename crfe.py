...
#v022 = CRFE:  cleaned up some more, corrected p-value computation, deleted average positive of perturbed genes, deleted parameter distribution learnt from MCMC
...
class CRFE:
    ...
    def __init__(self, repeats, lower_cutoff, upper_cutoff, belief, 
                 ...
                 seed=-1, max_belief=10,LEARN_PENALIZATION_PARAMETER=True,penalization_parameter=0.001,GET_INFO_ON_CURRENT_SETS=False,alpha=0.1,beta=0.25):        
        ...
        self.seed=seed
        self.number_different_beliefs=1#number_different_beliefs#LEARN_BELIEF doesn't work
        self.LEARN_BELIEF = False #doesn't work
        ...
        self.INCLUDE_ANNOTATIONS_IN_OUTPUT=False #huge output file if True
    ...
    def get_specific_rates(self, falserate, number, belief):
        '''This function is used to find the category-specific weights. 
        Falserate is the desired mean value of all weights'''
    ...
    def propose_MCMC_state(self,neighborhood_size):
        ...
        if r > self.proportion_parameter_change: #add, delete, or switch a term
            ...
            else: #change belief parameter, doesn't work
    ...
    def mean_position_perturbed_genes(self,list_of_terms):
        ...
        for term in list_of_terms:
            ...
            if T_sort[0]>=self.n_perturbed_genes: #if no perturbed gene is annotated to the term
                output.append(-1)
            else:
                output.append(np.mean(T_sort[T_sort<self.n_perturbed_genes])/self.n_perturbed_genes)
        return output
    ...
    def p_values_of_list(self, list_of_terms):
        ...
        for term in list_of_terms:
            ...
            rv = hypergeom(self.n_genes,n_annotated_genes,self.n_perturbed_genes)
            ps.append(1-rv.cdf(len(annotated_genes & perturbed_genes)))
        return ps
    ...
    def writeOut(self, filename, IS, mean_max_score, std_max_score, enriched, ps, mean_gene_level, mean_position_perturbed_genes, method, avg_posteriors, std_posteriors):
        ...
        self.belief,self.max_belief, method[0], self.repeats, self.burnin_steps, self.MCMC_steps, 
...
def main():
    ...
    p.add_option('--max_belief', '-B', default='10', help='Maximal belief, sets the maximal bounds for the false positive and false negative rate (default 10)')
    ...
    try:
        max_belief = int(options.max_belief)
    except ValueError:
        max_belief = 10
    try:
    ...
    try:
        ...
        if proportion_parameter_change>1 or proportion_parameter_change<0:
