...
cats.append(gr.categories([C_FA],m.T,[[0 for i in range(len(C_FA))]],m.levels_ordinal,m.number_of_genes-len(m.G[-1]),m.number_of_genes,10,cutoff=threshold,kind=3))
cats.append(gr.categories([C_GOrilla],m.T,[[0 for i in range(len(C_GOrilla))]],m.levels_ordinal,m.number_of_genes-len(m.G[-1]),m.number_of_genes,10,cutoff=threshold,kind=3))

colorlist=[gr.rgb(212,175,255),gr.rgb(176,232,249),gr.rgb(255,224,191),gr.rgb(255,176,176),gr.rgb(169,235,196),gr.rgb(153,76,0)]
legend.extend(['FuncAssociate2.0','GOrilla'])
a=gr.plotte_categories_explained2(cats,m,len(m.G[-1]),min_level,legend=legend,fontsize=15,title="",identifier="",MCMC_threshold=threshold,colors=colorlist)
