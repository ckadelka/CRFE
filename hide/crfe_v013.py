    ...
    def initialize(self,initial_set_of_terms=[]):
        ...
        if self.proportion_parameter_change>0:
            if s == self.n_perturbed_genes: #if each gene is in its own category, use simple formula for self.alpha_beta_max_upperbound
                self.alpha_beta_max_upperbound = (1+self.belief) / (1+3*self.belief)
            else:
                denominator = 0
                for i in range(1,s+1):
                    denominator += (s-i+(i-1)*self.belief)*self.n_perturbed_genes_per_category[i-1]
                self.alpha_beta_max_upperbound = 1./(1 + ((s-1)*self.belief*self.n_perturbed_genes)*1./denominator)
            if self.alpha_beta_max<=self.alpha_beta_max_upperbound:
                ...
                print('\n!!! Warning:\nThe chosen upper bound (%f) for alpha and beta, the FPR and FPR, is higher than the maximal possible upper bound (%f) so that genes annotated to activated terms are never penalized more than those not annotated to activated terms (i.e. need to ensure 1-beta_k > alpha). Upper bound set to %f\n' % (self.alpha_beta_max,self.alpha_beta_max_upperbound,self.alpha_beta_max_upperbound))
