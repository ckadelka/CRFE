...
for which in range(10,14):
    ...
    try:
        if disease_name.lower()[:3]=='gds':
            o = tools_new.CRFE(1, s, cutoff, top_cutoff, belief, 0.559, 'probs', burnin, steps, 0.5,0.2,20,20,'datasets/gene_file_'+disease_name+'.txt',0,
        ...
        if disease_name.lower()[:3]=='gds':
            o = tools_new.CRFE(s, cutoff, top_cutoff, alpha, beta, prob, belief, weight_kind, category_kind, 0.559, 'probs', burnin, steps, 0.5,0.2,20,20,'datasets/gene_file_'+disease_name+'.txt',0,
                        'ONTOLOGY_PARSER/annotation_file_human_association_human_biological_process.txt',
                        'output1/', 'NCBI_'+disease_name)                 

    
    if 'NCBI' in o.out_str:
        combined=''
        if int(o.gene_file.split('/')[-1][13:17]) in [3004,4419,4610,4974]:
            o.go_file='ONTOLOGY_PARSER/annotation_file_human'+combined+'_association_human_biological_process.txt'
        elif int(o.gene_file.split('/')[-1][13:17]) in [2969,3866]:
            o.go_file='ONTOLOGY_PARSER/annotation_file_yeast'+combined+'_association_yeast_biological_process.txt'
        elif int(o.gene_file.split('/')[-1][13:17]) in [1686]:
            o.go_file='ONTOLOGY_PARSER/annotation_file_fly'+combined+'_association_fly_biological_process.txt'
        elif int(o.gene_file.split('/')[-1][13:17]) in [3928]:
            o.go_file='ONTOLOGY_PARSER/annotation_file_rat'+combined+'_association_rat_biological_process.txt'
        elif int(o.gene_file.split('/')[-1][13:17]) in [3246,4423,4929,5092]:
            o.go_file='ONTOLOGY_PARSER/annotation_file_mouse'+combined+'_association_mouse_biological_process.txt'
        elif int(o.gene_file.split('/')[-1][13:17]) in [3216,3933]:
            o.go_file='ONTOLOGY_PARSER/annotation_file_arabidopsis'+combined+'_association_arabidopsis_biological_process.txt'
        #o.out_str+='_'+o.gene_file.split('_')[-1][:-4]
                  
    o.get_sets_and_genes()
    o.load_all()
    ...
    large_goids_int=map(int,o.large_goids)
    #if large_goids exists
    C_FA=[]
    for c in C_help:
        try:
            C_FA.append(m.term_names.index(o.term_names[large_goids_int.index(c)]))
        except:
            try:
                ind=large_goids_int.index(c)
                if '&' in o.term_names[ind]:
                    names=o.term_names[ind].split(' & ')
                    for name in names:
                        try:
                            C_FA.append(m.term_names.index(name))
                            break
                        except:
                            continue
            except:
    ...
    for c in C_help:
        try:
            C_GOrilla.append(m.term_names.index(o.term_names[large_goids_int.index(c)]))
        except:
            try:
                ind=large_goids_int.index(c)
                if '&' in o.term_names[ind]:
                    ...
                    for name in names:
                        try:
                            C_GOrilla.append(m.term_names.index(name))
                            break
                        except:
                            continue
            except:
    ...
    for c in C_help:
        try:
            C_GSEA.append(m.term_names.index(o.term_names[large_goids_int.index(c)]))
        except:
            try:
                ind=large_goids_int.index(c)
                if '&' in o.term_names[ind]:
                    ...
                    for name in names:
                        try:
                            C_GSEA.append(m.term_names.index(name))
                            break
                        except:
                            continue
            except:
    ...
    colorlist=[gr.rgb(212,175,255),gr.rgb(176,232,249),gr.rgb(255,224,191),gr.rgb(255,176,176),gr.rgb(169,235,196),gr.rgb(153,76,0),gr.rgb(55,240,40)]
    termss.extend([C_FA,C_GOrilla,C_GSEA])
    meanss.extend([[0 for i in range(len(C_FA))],[0 for i in range(len(C_GOrilla))],[0 for i in range(len(C_GSEA))]])
    legend.extend(['FuncAssociate2.0','GOrilla','GSEA'])
