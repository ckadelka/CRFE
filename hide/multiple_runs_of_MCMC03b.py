...
weight_kind=1
category_kind=0
data1=[[[0 for iii in range(3)] for ii in range(2)] for i in range(10)]
data_full=[[[0 for iii in range(3)] for ii in range(2)] for i in range(10)]
data_G=[[[0 for iii in range(3)] for ii in range(2)] for i in range(10)]
...
for which in [1]:
    for s in [0]:
        ...
        for belief in [2,5,10]:
            ...
            print which,s,count,nsim,max(help),min(help)
            data_full[which][s][count]=different_cutoffs(C,T,MCMC_distr,levels_ordinal,N=100)
            data1[which][s][count]=stats(C,T,MCMC_distr,levels_ordinal,cutoff=0.5)#different_cutoffs(C,T,MCMC_distr,levels_ordinal,N=100)
            data_G[which][s][count]=(lug,len(G[-1]))
            count+=1
