...
#v014:  include belief parameter among the set of learned parameters, got rid of self.max_nr_alpha_plus_nr_beta in MCMC
...
class CRFE:
    ...
    def propose_MCMC_state(self,neighborhood_size):
        ...
        if r > self.proportion_parameter_change: #add, delete, or switch a term
            ...
            if r < self.proportion_parameter_change/2: #change alpha
            ...
            elif r < self.proportion_parameter_change*2/2: #change beta
                ...
                # #self.old_NP_log_alpha = self.NP_log_belief
                # self.nr_alpha=int(random.random()*self.A)
                # self.alpha = self.possible_values_for_alpha_and_beta[self.nr_alpha]
                # self.log_alphas=self.possible_log_alphas[self.nr_alpha]
                # self.NP_log_alpha = sum([a*(b-c) for a,b,c in zip(self.log_alphas,self.n_perturbed_genes_per_category,self.n_explained_perturbed_genes_per_category)])#np.dot(self.log_alphas,self.n_perturbed_genes_per_category-self.n_explained_perturbed_genes_per_category)
    ...
    def undo_proposal(self):
        if self.term_id_to_toggle != -1: #addition or deletion of a term was not accepted
            ...
            #self.belief = #self.possible_values_for_alpha_and_beta[self.nr_beta]
            #self.log_1_m_betas=self.possible_log_1_minus_betas[self.nr_beta]
            #self.EP_log_1_minus_beta=self.old_EP_log_1_minus_beta            
    ...
    def initialize(self,initial_set_of_terms=[]):
        ...
        if self.proportion_parameter_change>0:
            ...
            self.possible_log_alphas = [np.array(el) for el in np.log(self.possible_alphas_and_betas)]
            self.possible_log_1_minus_betas = [np.array(el) for el in np.log(1-self.possible_alphas_and_betas)]
