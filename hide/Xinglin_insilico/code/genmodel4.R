...
human_anno = d[sapply(human, length) %in% c(20:100)]
human = human[sapply(human, length) %in% c(20:100)]
...
design = data.frame(c(rep(0.25, 4), rep(0.4, 4)), 
                    ...
                    c(rep(c(10,100),4)))
colnames(design) = c("alpha", "beta", "belif", "n")
design = data.frame(.4,.25,10,100)
colnames(design) = c("alpha", "beta", "belif", "n")
## human
write.table(human_anno, file = "/Users/JasonJia/Desktop/CRFE/Xinglin_insilico/data/human4/human_anno.txt", col.names = FALSE, row.names = F, quote = F)
for (i in 1:nrow(design)) {
  for (j in 1:5) {
    set.seed(j)
    d = unlist(design[i,])
    result = letsgen4(data = human, alpha = d[1], beta = d[2], belif = d[3], n = d[4])
    write.table(result$gene_rank, file =paste0("/Users/JasonJia/Desktop/CRFE/Xinglin_insilico/data/human4/human", paste(d[1]), "_", paste(d[2]), "_",
                                               paste(d[3]),"_", paste(d[4]), "_seed_",paste(j)  ,".txt"), 
                sep ="\t", row.names = F, quote = F, col.names = F)
    write.table(result$activated_terms, file = paste0("/Users/JasonJia/Desktop/CRFE/Xinglin_insilico/data/human4/human", paste(d[1]), "_", paste(d[2]), 
