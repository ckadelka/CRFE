...
preplot_real_proportion = function(data_dir, crfe_dir, anno, top_percent, threshold){
  crfe = read.csv(crfe_dir, sep = "\t") # table from CRFE
  term = crfe[,9] # CRFE ranked terms
  all_genes = read.csv(data_dir, sep = "\t",header = F) # expression file, ranked genes
  U = all_genes[floor(length(all_genes$V1)*threshold):length(all_genes$V1),1]
  U = unique(U)
  P = all_genes[1:floor(length(all_genes$V1)*threshold),1]
  P = unique(P)
  
  n_X = floor(length(P)*top_percent/100) # number of top X percent of the perturbed genes
  X = all_genes[all_genes$V1 %in% P,1][1:n_X]# top X percent perturbed genes
