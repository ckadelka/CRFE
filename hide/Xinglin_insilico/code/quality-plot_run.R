...
for (k in 1:nrow(design)) {
  ...
  plot(x, c(1,b1[2:1000]+3), pch = " ", main = paste0("human 5 rep, 10 repeat, α=",paste(des[1])," β=",paste(des[2]), " b=",paste(des[3])," n=", paste(des[4])),
       xlab = "Proportion of Top 30% Perturbed Genes that are Explained", ylab = "Quality score")
  ...
  legend("topright", legend=c("CRFE b=1","CRFE b=2", "CRFE b=5", "CRFE b=10"), fill = color[1:4])
}
...
#par(mfrow = c(1,2))
for (i in 2) {
  dn = paste0("/temp/human0.1_0.4_1_20_seed_", paste(i), ".txt")
  gd = paste0("/Users/JasonJia/Desktop/CRFE/Xinglin_insilico/data/human4/human0.1_0.4_1_20_seed_", paste(i), ".txt")
  
  b1 = preplot_real_proportion(crfe_dir = paste0(dd,"b1",dn),
                               data_dir = gd,
                               anno = human,
                               top_percent = tp,
                               threshold = thres)
  
  b2 = preplot_real_proportion(crfe_dir = paste0(dd,"b2",dn),
                               data_dir = gd,
                               anno = human,
                               top_percent = tp,
                               threshold = thres)
  
  b5 = preplot_real_proportion(crfe_dir = paste0(dd,"b5",dn),
                               data_dir = gd,
                               anno = human,
                               top_percent = tp,
                               threshold = thres)
  b10 = preplot_real_proportion(crfe_dir = paste0(dd,"b10",dn),
                                data_dir = gd,
                                anno = human,
                                top_percent = tp,
                                threshold = thres)
  ...
  color = c("red", "blue", "black","purple", "green")
  plot(seq(0, 1, 0.001), seq(0, 30, 0.03),
       pch = " ", main = "random, threshold30%,repeat10,adjusted_ab_max",
       xlab = "Proportion of Top 100% Perturbed Genes that are Explained", ylab = "Quality score", xlim = c(0,1))
