    ...
    for s,belief in zip([0,0,0,1],[10,5,2,1]): #s
        ...
        for ind in [0]: #min_level
            for j in [0]: #belief
                ...
                data1[hdhsfs][0][ind][count]=gr.categories(C,m.T,MCMC_distr,m.levels_ordinal,m.number_of_genes-len(m.G[-1]),m.number_of_genes,10,cutoff=MCMC_threshold,kind=3)
                ...
                if count==3:#ss==[0,1] and s==1 and j==1:
                    print "HABBBASD"
                    ...
                    gr.plotte_categories_explained2([data1[hdhsfs][0][ind][0],data1[hdhsfs][0][ind][1],data1[hdhsfs][0][ind][2],data1[hdhsfs][0][ind][3]],m,len(m.G[-1]),min_level,legend=[r"CRFE, 0.1", r"CRFE, 0.2", r"CRFE, 0.3",r"CRFE, 0.5"],title=title,identifier=identifier,ymax=ymax,MCMC_threshold=MCMC_threshold,colors=['k','b','r','g'])
