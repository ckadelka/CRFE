...
class CRFE:
    ...
    def __init__(self, repeats, nr_categories, lower_cutoff, upper_cutoff, belief, threshold, threshold_type, burnin_steps, MCMC_steps, alpha_beta_max, proportion_parameter_change, A, P, gene_file, annotations_file, output_folder, out_str='', seed=-1):        
    ...
    def propose_MCMC_state(self,neighborhood_size):
        ...
        if r > self.proportion_parameter_change: #add, delete, or switch a term
            ...
            if proposal < self.n_terms: #add or delete a term
                ...
                #learn prob every time because it is easy
                self.old_nr_prob=self.nr_prob
                self.old_prob=self.prob                
                self.nr_prob=min(max(1,self.n_terms-self.number_of_unselected_sets),self.P)
                self.prob=self.nr_prob*1./self.n_terms
            ...
            if r < self.proportion_parameter_change/2: #change alpha
                self.old_nr_alpha = self.nr_alpha
                self.old_NP_log_alpha = self.NP_log_alpha
                self.nr_alpha=int(random.random()*min(self.max_nr_alpha_plus_nr_beta-self.nr_beta,self.A))
                self.alpha = self.possible_values_for_alpha_and_beta[self.nr_alpha]
                self.log_alphas=self.possible_log_alphas[self.nr_alpha]
                ...
                self.ALPHA_WAS_CHANGED = True
            else: #change beta
                self.old_nr_beta = self.nr_beta
                self.old_EP_log_1_minus_beta=self.EP_log_1_minus_beta
                self.nr_beta=int(random.random()*min(self.max_nr_alpha_plus_nr_beta-self.nr_alpha,self.A))
                self.beta = self.possible_values_for_alpha_and_beta[self.nr_beta]
                self.log_1_m_betas=self.possible_log_1_minus_betas[self.nr_beta]
    ...
    def undo_proposal(self):
        if self.term_id_to_toggle != -1: #addition or deletion of a term was not accepted
            ...
            self.prob=self.old_prob
            self.nr_prob=self.old_nr_prob            
        ...
        elif self.ALPHA_WAS_CHANGED: #change of alpha was not accepted
            self.nr_alpha = self.old_nr_alpha
            self.alpha = self.possible_values_for_alpha_and_beta[self.nr_alpha]
            self.log_alphas = self.possible_log_alphas[self.nr_alpha]
            self.NP_log_alpha = self.old_NP_log_alpha            
        else: #change of beta was not accepted
    ...
    def get_score(self):
        ...
        if self.prob!=1:
            L += math.log(self.prob) * (self.n_terms - self.number_of_unselected_sets) + math.log(1.0-self.prob) * self.number_of_unselected_sets
        return L
    ...
    def initialize(self,initial_set_of_terms=[]):
        ...
        self.n_explained_perturbed_genes_per_category = [0]*s
        ...
        if self.proportion_parameter_change>0:
            ...
            for i in range(1,s+1):
                numerator += (s-i+(i-1)*self.belief)*self.n_perturbed_genes_per_category[i-1]
            self.max_alpha_plus_beta = numerator*1./((s-1)*self.belief*self.n_perturbed_genes)
            if self.alpha_beta_max<self.max_alpha_plus_beta:
                self.possible_values_for_alpha_and_beta = list(np.linspace(0,self.alpha_beta_max,self.A+1)[1:])
            else:
                self.possible_values_for_alpha_and_beta = list(np.linspace(0,self.max_alpha_plus_beta,self.A+2)[1:-1])
            ...
            self.max_nr_alpha_plus_nr_beta = int(self.max_alpha_plus_beta//self.possible_values_for_alpha_and_beta[0]-2) 
            
            self.possible_alphas_and_betas=np.array([self.get_specific_rates(self.possible_values_for_alpha_and_beta[i]) for i in range(self.A)])
            self.possible_log_alphas = [list(el) for el in np.log(self.possible_alphas_and_betas)]
            self.possible_log_1_minus_betas = [list(el) for el in np.log(1-self.possible_alphas_and_betas)]
            ...
            self.prob=min(max(len(initial_set_of_terms),1),self.P)*1./self.n_terms
            self.nr_prob=min(max(len(initial_set_of_terms),1),self.P)-1
        else:
    ...
    def MCMC_alg(self,initial_set_of_terms=[],verbose=0):
        ...
        while self.step < total_steps:              
            ...
            if self.step%PRINT_AFTER_X_STEPS==0:
                if verbose>0:
                    ...
                    if verbose>1:
                        print(self.step, self.score, self.alpha,self.beta, self.nr_prob, self.EP_log_1_minus_beta,self.NP_log_alpha,self.EU,self.NU)
                learned_params.append([self.step,self.score,self.alpha,self.beta,str(int(round(self.prob*len(self.T))))+"/"+str(self.n_terms),self.prob,self.n_terms - self.number_of_unselected_sets])
        ...
        if self.proportion_parameter_change>0:            
            ...
            hist_prob=([p*1./self.MCMC_steps for p in prob_count],range(1,self.P+1))
                              
        if self.proportion_parameter_change>0:
            return (self.max_score, Final_Distribution, learned_params, hist_alpha, hist_beta, hist_prob)
    ...
    def writeOut_MCMC(self, filename, learned_params, hist_alpha=[], hist_beta=[], hist_prob=[]):
        nr_terms=len(self.T)
        filename=self.output_folder+'mcmc_'+filename
        if not os.path.exists(self.output_folder):
            os.makedirs(self.output_folder)
        f_out = open(filename+'.html', 'w')
        names=['Iteration step','Likelihood value','alpha (FPR)', 'beta (FNR)', 'q (rational)','q (decimal)', '|C|']
        ...
        strings+="<h2>Learnt parameter values</h2><table>"
        ...
        strings+="<tr valign=\"top\"><td>Penalization parameter (q):</td><td>%s</td></tr>" % (self.prob)
        strings+="<tr valign=\"top\"><td>Penalization per term:</td><td>%s</td></tr>" % (-math.log(self.prob/(1-self.prob)))
        strings+="</table>"
        ...
        for j in range(len(learned_params)):
            ...
            for i in range(7):
                strings+="<td style=\"width:150px;\">%s</td>" % (str(round(learned_params[j][i],3) if i in [1,2,3] else (round(learned_params[j][i],5) if i==5 else learned_params[j][i])))
            strings+="</tr>\n"
        f_out.write(strings)
        f_out.write("</table>\n</body>\n</html>")
        f_out.flush()
    ...
    def writeOut(self, filename, IS, mean_max_score, std_max_score, enriched, ps, mean_gene_level, method, avg_posteriors, std_posteriors):
        ...
        str2 = "alpha (FPR):\t%s\nbeta (FNR):\t%s\nq (penalization parameter):\t%s\n" % ( self.alpha, self.beta, self.prob)
        text.append(str2+"\n")
        ...
        for i in range(len(mean_gene_level)):
            ...
            out_list[i]+="\t" + str(round(mean_gene_level[i],3)) + "\t" + str(enriched[i][1])
            if self.show_genes_in_output:
                out_list[i]+='\t'
                for j in ind:
    ...
    def runMe(self, verbose=False, parameter_initial_MCMC_set=0):        
        ...
        if self.proportion_parameter_change>0:
            hist_alphas,hist_betas,hist_probs=[],[],[]
        max_scores,posteriors=[],[]
        for i in range(self.repeats):
            (max_score, MCMC_Distr, learned_params, hist_alpha, hist_beta, hist_prob)=self.MCMC_alg(initial_set_of_terms=initial_set_of_terms,verbose = 2 if verbose else -1)
        ...
        if self.proportion_parameter_change>0:
            ...
            self.prob=(np.argmax(mean_hist_probs)+1)*1./self.n_terms
        ...
        if self.proportion_parameter_change>0:        
            self.writeOut_MCMC(filename, learned_params, [mean_hist_alphas,self.possible_values_for_alpha_and_beta],[mean_hist_betas,self.possible_values_for_alpha_and_beta],[mean_hist_probs,np.arange(1,self.P+1)])
...
#    main()
    ...
    m = CRFE(repeats, nr_categories, lower_cutoff, upper_cutoff, belief, 
                                        0.4, 'levels', burnin, steps, 1,0.2,20,20, gene_file,
