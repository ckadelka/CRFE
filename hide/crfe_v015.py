...
class CRFE:
    ...
    def __init__(self, repeats, nr_categories, lower_cutoff, upper_cutoff, belief, threshold, threshold_type, burnin_steps, MCMC_steps, alpha_beta_max, proportion_parameter_change, A, P, gene_file, annotations_file, output_folder, out_str='', seed=-1, max_belief=10, B=10,LEARN_BELIEF=True):        
    ...
    def initialize(self,initial_set_of_terms=[]):
        ...
        if self.proportion_parameter_change>0:
            if s == self.n_perturbed_genes: #if each gene is in its own category, use simple formula for self.alpha_beta_max_upperbound
                self.alpha_beta_max_upperbound = (1+self.max_belief) / (1+3*self.max_belief)
            else:
                denominator = 0
            ...
            (self.belief,self.nr_belief)=self.discretize(self.possible_values_for_belief,5)
        ...
        while self.step < total_steps:              
            ...
            if self.step%PRINT_AFTER_X_STEPS==0:
                if verbose>0:
                    ...
                    if verbose>1:
                        print(self.step, self.score, self.alpha,self.beta, self.belief, self.nr_prob, self.EP_log_1_minus_beta,self.NP_log_alpha,self.EU,self.NU)
                learned_params.append([self.step,self.score,self.alpha,self.beta,self.belief,str(int(round(self.prob*len(self.T))))+"/"+str(self.n_terms),self.prob,self.n_terms - self.number_of_unselected_sets])
