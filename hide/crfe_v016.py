...
class CRFE:
        ...
        if r > self.proportion_parameter_change: #add, delete, or switch a term
            ...
            if proposal < self.n_terms: #add or delete a term
                ...
                if self.LEARN_PENALIZATION_PARAMETER:
                    ...
                    self.old_prob=self.penalization_parameter                
                    self.nr_prob=min(max(1,self.n_terms-self.number_of_unselected_sets),self.P)
                    self.penalization_parameter=self.nr_prob*1./self.n_terms
    ...
    def undo_proposal(self):
        if self.term_id_to_toggle != -1: #addition or deletion of a term was not accepted
            ...
            if self.LEARN_PENALIZATION_PARAMETER:
                self.penalization_parameter=self.old_prob
        ...
        while self.step < total_steps:              
            ...
            if self.step%PRINT_AFTER_X_STEPS==0:
                if verbose>0:
                    ...
                    if verbose>1:
                        print(self.step, self.score, self.alpha,self.beta, self.belief, self.nr_prob if self.LEARN_PENALIZATION_PARAMETER else 0, self.EP_log_1_minus_beta,self.NP_log_alpha,self.EU,self.NU)
    ...
    def runMe(self, verbose=False, parameter_initial_MCMC_set=0):        
        ...
        if self.proportion_parameter_change>0:
            ...
            if LEARN_PENALIZATION_PARAMETER:
...
#    main()
    ...
    m = CRFE(repeats, nr_categories, lower_cutoff, upper_cutoff, belief, 
                                      ...
                                      LEARN_PENALIZATION_PARAMETER=False,penalization_parameter=0.001,GET_INFO_ON_CURRENT_SETS=True)
