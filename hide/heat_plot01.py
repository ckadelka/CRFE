...
m = tools.GeneSetRankedEnrichment(2, s, 5, 200, 0.1, 
                            0.25, 1e-5, 5, 0, 1.2, 1000, 1000, 100, int(5*1e4), 0.5,0.2,20,20,'datasets/disease_obesity.txt',
                            'human-taxon-id-gene2go-with-annotation-status-closed-bioprocess-only.csv',
                            'output1/', 'DISEASE')
#m = tools.GeneSetRankedEnrichment(2, s, 5, 200, 0.05, 
...
#                            0.25, 1e-10, 5, 0, 0.559, 1000, 1000, 20000, 5*int(1e5), 0.5,0.2,20,20,'bkz_gene_list.csv',
#                            'human-taxon-id-gene2go-with-annotation-status-closed-bioprocess-only.csv',
#                            'output1/', 'HDF')
#Usage:
...
for i in range(len(C)):
    ...
    for j in range(len(G)):
        mat[-1].append(len(set(G[j])&set(T[C[i]])))
...
if MCMC:
    string='GO term \t MCMC density \t Total annotations per term & p-value\t'
else:
    string='GO term \t Total annotations per term & p-value\t'    
for val in ccG:
    string+=str(val)+'\t'
f_out.write(string[:-1]+'\n')
for i in range(len(fold)):
